...
class HtmlReporter extends SimpleReporter {
    ...
    function paintFooter($test_name) {
        ...
        print " тестов завершено:\n";
        print "<strong>" . $this->getPassCount() . "</strong> успешно, ";
        print "<strong>" . $this->getFailCount() . "</strong> провалов и ";
        print "<strong>" . $this->getExceptionCount() . "</strong> исключений.";
        print "</div>\n";
        print "</body>\n</html>\n";
    }
    ...
    function paintFail($message) {
        ...
        print "<span class=\"fail\">Провал</span>: ";
    ...
    function paintError($message) {
        ...
        print "<span class=\"fail\">Исключение</span>: ";
        $breadcrumb = $this->getTestList();
        array_shift($breadcrumb);
        print implode(" -&gt; ", $breadcrumb);
    ...
    function paintSkip($message) {
        ...
        print "<span class=\"pass\">Пропущено</span>: ";
        $breadcrumb = $this->getTestList();
        array_shift($breadcrumb);
        print implode(" -&gt; ", $breadcrumb);
        print " -&gt; " . $this->_htmlEntities($message) . "<br />\n";
...
class JsonReporter extends SimpleReporter {
    ...
    function paintFooter($test_name) {
        ...
        print json_encode(array(
            "status" => $status_ok, 
            "count" => $this->getTestCaseCount(),
            "passes" => $this->getPassCount(),
            "failures" => $this->getFailCount(),
            "exceptions" => $this->getExceptionCount(),
            "text" => $this->getTestCaseProgress(),
            "errors" => $this->errors,
        ));
    ...
    function paintFail($message) {
        ...
        $html =  $this->getFailCount() . ") $message ";
        $breadcrumb = $this->getTestList();
        array_shift($breadcrumb);
        $html .= " in " . implode(" in ", array_reverse($breadcrumb));
        $html .= "<br/>";
        
        $this->errors[] = array("type"=>1, "text"=>$html);
    }
    ...
    function paintError($message) {
        parent::paintError($message);
        $html .=  "Exception " . $this->getExceptionCount() . "! $message <br/>";
        $breadcrumb = $this->getTestList();
        array_shift($breadcrumb);
        $html .=  " in " . implode(" in ", array_reverse($breadcrumb));
        $html .= "<br/>";
        
        $this->errors[] = array("type"=>2, "text"=>$html);
    }
    ...
    function paintException($exception) {
        ...
        $message = 'Unexpected exception of type [' . get_class($exception) .
                ...
                '] in ['. $exception->getFile() .
                ' line ' . $exception->getLine() . ']';
        $html .= "Exception " . $this->getExceptionCount() . "!\n$message\n";
        $breadcrumb = $this->getTestList();
        array_shift($breadcrumb);
        $html .= " in " . implode(" in ", array_reverse($breadcrumb));
        $html .= "<br/>";
        
        $this->errors[] = array("type"=>3, "text"=>$html);
    }
