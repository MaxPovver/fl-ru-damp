...
<div class="b-layout b-layout_pad_10 b-layout_bord_e6 b-layout_relative b-layout_margbot_10">
    ...
    <div class="b-layout__txt b-layout__txt_fontsize_15 b-layout__txt_padleft_70 b-layout__txt_padleft_null_iphone b-layout__txt_padtop_2">ѕрофессиональный аккаунт на <? if($type['day']) { ?><?= $type['day']?> <?= ending($type['day'], 'день', 'дн€', 'дней')?><? } elseif ($type['week']) { ?><?= $type['week']?> <?= ending($type['week'], 'неделю', 'недели', 'недель')?><? } else { ?><?= $type['month']?> <?= ending($type['month'], 'мес€ц', 'мес€ца', 'мес€цев')?><? } ?></div>
</div>