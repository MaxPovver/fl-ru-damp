...
if(hljs) {
    ...
    Object.append(MooEditable.Actions, {
        codeBlock: {
            ...
            command: function(menulist, name){
                var s, r, nd, ts, tsid, txt, _cb, pp, is_last, _p, prevNode, nextNode, nodeElements;
                if(name == '0') { // конвертация code в обычный текст
                    ...
                    txt = txt.replace(/<p><br\s?\/?><\/p>/gi, '<p></p>');
                    ...
                    if (txt != s.getNode().get('html')) { // если сложное выделение (более одного тега)
                        ...
                        // в случае если в код преобразуется только часть строки, и при этом остаются куски текста до и/или после кода, их нужно обернуть в <p>
                        prevNode = _cb.previousSibling;
                        if (prevNode && prevNode.nodeType === 3 && prevNode.textContent.trim().length) {
                            new Element('p').wraps(prevNode);
                        }
                        nextNode = _cb.nextSibling;
                        if (nextNode && nextNode.nodeType === 3 && nextNode.textContent.trim().length) {
                            new Element('p').wraps(nextNode);
                        }
                        
                        // удаляем вложенность элементов p, которая, если не убрать, добавит по пустому тегу до и после кода
                        // вложенность получается если выделяется не целый параграф а только часть текста
                        var wrap = _cb.getParent();
                        if (wrap.get('tag') === 'p' && (Browser.Engine.webkit || Browser.opera || (Browser.ie && Browser.version >= 9))) {
                            wrap.getChildren().inject(wrap, 'before');
                            wrap.dispose();
                        }
                        
                        // в ИЕ параграфы появляются до и после кода, удаляем их
                        // параграфы появляются если были выделены целые параграфы
                        if (wrap.get('tag') === 'body' && Browser.ie && Browser.version >= 9) {
                    ...
                    // делаем чтобы вид пришел в нормальное состояние после всех манипуляций с DOM элементами
                    // например, в опере появляется лишняя строка, но при просмотре в отладчике на самом деле ничего нет
                    this.saveContent();
