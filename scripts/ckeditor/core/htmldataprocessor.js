...
(function() {
		...
		function maybeBogus( node, atBlockEnd ) {
					 ...
					 ( match = node.value.match( tailNbspRegex ) ) )
			{
				...
				if ( !isOutput ) {
					...
					if ( previous && ( previous.name == 'br' || previous.name == 'cut' ) )
						return true;
	...
	var protectElementRegex = /<(a|area|img|input|source)\b([^>]*)>/gi,
		...
		clearElementRegex = /<(.+?)\b([^>]*)>/gi,
		disableAttributeRegex = /\b(class|style)\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|(?:[^ "'>]+))/gi;
	...
	function protectAttributes( html ) {
		...
		// Clear ALL style and class attributes
		return html.replace( clearElementRegex, function( element, tag, attributes ) {
			return '<' + tag + attributes.replace( disableAttributeRegex, function( fullAttr, attrName ) {
				if(tag == 'p' && attrName == 'class' && fullAttr.indexOf('"code ') != -1) {
					return ' ' + fullAttr;
				}  
				return ' ';
			}) + '>';
