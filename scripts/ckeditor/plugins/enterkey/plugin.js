...
(function() {
	...
	CKEDITOR.plugins.enterkey = {
		...
		enterBr: function( editor, mode, range, forceMode ) {
            ...
            if(elm.is('a')) {
                var maskBR = '___BR___',
                    lineBreak = doc.createText( maskBR );

                range.deleteContents();
                range.insertNode( lineBreak );
                var html = elm.getHtml();
                if( html.indexOf(maskBR) == ( html.length - maskBR.length) ) {
                    var linkEndText = true;
                    elm.setHtml(html.replace(maskBR, ''));
                    range.setStartAt( elm, CKEDITOR.POSITION_AFTER_END );
                } else {
                    var linkEndText = false;
                    elm.setHtml(html.replace(maskBR, '<br/>'));
                }
            }
			...
			if ( !forceMode && isEndOfBlock && headerTagRegex.test( startBlockTag ) ) {
				...
				if(!linkEndText) range.deleteContents();
				range.insertNode( lineBreak );
