...
/*
 CVS:
  $Id: class_webdav_client.php,v 1.8 2004/08/25 20:50:31 chris Exp $
  $Author: chris $
  $Date: 2004/08/25 20:50:31 $
  $Revision: 1.8 $
*/
/**
...
class webdav_client {
  ...
  */
	...
	var $_errno;
	var $_errstr;
	var $_user_agent = 'php class webdav_client $Revision: 1.8 $';
	var $_crlf = "\r\n";
	var $_req;
	var $_resp_status;
	var $_parser;
	...
	*/
	function get_file($srcpath, $localpath) {
		...
		if ($this->get($srcpath, $buffer)) {
            ...
            //@todo: это портит кодировку в локальной файловой системе
...
/** 
		...
		if ($response['status']['http-version'] == 'HTTP/1.1' ||
			...
			if (strcmp($response['status']['status-code'],'207') == 0 ) {
				...
				if (strcmp($response['header']['Content-Type'], 'text/xml; charset="utf-8"') == 0) {
					...
					//unset($this->_xmltree[$this->_parser]);
					$this->_ls = array();    //верхнее откомменчено, потому что возникало переполнение памяти (не сбрасывалась предыдущая инфа о _ls
					$this->_xmltree = array();
					xml_parser_set_option($this->_parser,XML_OPTION_SKIP_WHITE,0);
					xml_parser_set_option($this->_parser,XML_OPTION_CASE_FOLDING,0);
 ...
 /**
	...
	*/
	...
	/**
	 * Проверяет наличие файла или директории на существование. 
	 * Внимание! если WebDav вернет код, отличный от 404, функция вернет true
	 * (если проверяется директория, то WebDav может вернуть 403 Forbidden, это нормально!)
	 *
	 * @param unknown_type $path
	 * @return unknown
	 */
	function is_file_light($path){
	
	    if (trim($path) == '') {
			$this->_error_log('Missing a path in method is_dir_light');
			return false;
		}
		$this->_path = $this->_translate_uri($path);
		...
		$this->_get_respond();
		$response = $this->_process_respond();
	    if (strcmp($response['status']['status-code'],'404') == 0 ) return false;
	    else return true;
	}
 ...
 /**
	...
	*/
	function _get_respond() {
		...
		stream_set_timeout($this->_fp, 10);
		//stream_set_blocking($this->_fp, FALSE );  //this will slow down responce, but will not loose data (as I think) 
		//$socket_state = socket_get_status($this->_fp);
		//print_r($socket_state);
		...
		switch(true) {
			case (preg_match('/Transfer\\-Encoding:\\s+chunked\\r\\n/',$header)):
				...
				do {
					...
					$chunk_size=intval(hexdec($chunk_size));                // convert to a number in decimal system
					if ($chunk_size > 0) {
 ...
 /**
	...
	*/
	function _process_respond() {
		...
		if ($this->_debug) $this->_error_log('_process_respond: ' . var_export($ret_struct,true));
		return $ret_struct;
