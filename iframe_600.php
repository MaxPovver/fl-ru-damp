...
<html>
	...
	<body>
		<div class="top-banner"><span class="middled">
	        ...
	        if ($g_page_id) {
	            $banners = new banners;
				$banner = $banners->ViewBanner($g_page_id, 1);
	        }
	        if ($banner) {
	        if ($banner['code']) { echo $banner['code']; } else { ?>
	            ...
	            <? } else { ?>
	                <script type="text/javascript">
	                    swfobject.embedSWF("<?=WDCPREFIX?>/banners/<?=$banner['filename']?>?link1=/banners/click.php?id=<?=$banner['id']?>", "TopBannerSwf", "<?=$banner['width']?>", "<?=$banner['height']?>", "9.0.0", "/scripts/expressInstall.swf", false, {wmode:"opaque"});
	                </script>
	                <div id="TopBannerSwf" style="height:<?=$banner['height']?>px"><img src="<?=WDCPREFIX?>/banners/<?=$banner['stat_fname']?>" alt="" width="<?=$banner['width']?>" height="<?=$banner['height']?>" /></div>
	            <? } ?>
	            <?=$banner['pixel']?>
	        <? } } else { ?><img src="/images/dot_grey.gif" alt="" width="600" height="60" />
	        <? } ?></span>
		</div>
	</body>
</html>