...
function fPopCalendar(evt, pDoc, dateCtrl,range,posLayerId,posCtrl,posOffset) {
	var dc=dateCtrl;
    ...
    var ls = pDoc.getElementsByTagName('body')[0].getElementsByTagName('div');    
    var dX = 0;	    
    for (var i = 0; i < ls.length; i++) {
        if (ls[i].className == 'body') {    
            dX = Math.round((screen.width - ls[i].offsetWidth)/2);
            break; 
        }
    }
    p[0] = evt.clientX - dX;
...
function fSetDate(y,m,d,taint,e) {
    ...
    if ( typeof(parent.messages_spam) != 'undefined' ) {
        parent.messages_spam.onCalendarSetDate(gdCtrl.id);
    }
