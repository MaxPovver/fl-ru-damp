...
$sql = "SELECT l_frl, extract(day from date) as day FROM stat_data WHERE date>=? AND date<=?";
$q = $DB->rows($sql, $syear."-".$smonth."-01", $syear."-".$smonth."-".$days);
$is_null = 1;
if($q) {
    foreach($q as $s) {
        if($s['l_frl']!=0) $is_null = 0;
        $data[$s['day']] = $s['l_frl'];
    }
}
...
$Test->drawTitle(50,22,"Живые фрилансеры",50,50,50,585);
$Test->Stroke();
?>
