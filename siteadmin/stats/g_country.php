...
function getOP($op, $date_from='2006-10-10', $date_to='now()', $bYear=false) {
	...
	if ($bYear) {
        $sql = "SELECT COUNT(*) as cnt, to_char(reg_date,'MM') FROM users WHERE country=".$op." AND reg_date >= '".$date_from."' AND reg_date < '".$date_to."' GROUP BY to_char(reg_date,'MM') ORDER BY to_char(reg_date,'MM')";
	}
	else {
        $sql = "SELECT COUNT(*) as cnt, extract(day from reg_date) as _day FROM users WHERE country=".$op." AND reg_date >= '".$date_from."' AND reg_date < '".$date_to."'::date+'1day'::interval GROUP BY _day ORDER BY  _day";
	}
	$res = $DB->rows($sql);
	return $res;
}
...
//echo count($countr).'-'.count($graphValues);
$imgHeight += $graphStyle[0]['max'] + (count($graphValues))*15+60; //прибавляем промежутки к максимальной высоте графика$imgWidth = $iMax*$iBarWidth+100;
$imgWidth = $iMax*$iBarWidth+100;
//if(count($countr)==1) $imgHeight += 60; else 
...
for($i=0; $i<count($countr); $i++) {
    $graphStyle[$i+1]['text'] = $countr[$i]['country_name'];
}
		...
		if ($graphValues[$i][$j]) {
            ...
            if($i!=0) {
                ...
                for($k=1; $k<count($graphValues); $k++) {
                    $addD = ($k == 8)?2:1;
                    $iCount += round($graphValuesV[$k][$j]/$addD);
                }
                imagefttext($image, '7', 0, $j*$iBarWidth + 100+2, $imgHeight-$iHeight-$graphValues[$i][$j]-2, $color, $sFont, round($graphValuesV[$i][$j]));
            }
		}
