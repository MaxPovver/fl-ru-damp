...
$sql = "SELECT main, l_main, extract(day from date) as day FROM stat_data_by_spec WHERE date>=? AND date<=? AND spec=?";
$q = $DB->rows($sql, $syear."-".$smonth."-01", $syear."-".$smonth."-".$days, $sspec);
$is_null = 1;
if($sspec!=0) {
    if($q) {
        foreach($q as $s) {
            if($s['main']!=0 || $s['l_main']!=0) $is_null = 0;
            $data1[$s['day']] = $s['main'];
            $data2[$s['day']] = $s['l_main'];
        }
    }
}
...
$DataSet->SetSerieName("Всего","S1");
$DataSet->SetSerieName("Живых","S2");
...
$Test->drawTitle(50,22,"Основная специализация",50,50,50,585);
$Test->Stroke();
?>
