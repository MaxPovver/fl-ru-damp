...
function getOP($op, $date_from='2006-10-10', $date_to='now()', $bYear=false) {
	...
	if ($bYear) {
		$sql = "SELECT count(*) as cnt to_char(reg_date,'MM') as _day FROM $op WHERE reg_date >= '".$date_from."' AND reg_date < '".$date_to."'    GROUP BY to_char(op_date,'MM') ORDER BY to_char(reg_date,'MM')";
	}
	else {
        $res = array();
        $n = 0;
        ...
        for($i=1; $i<=$iMaxDays; $i++) {
		    ...
		    $sql = "SELECT l_$op as cnt FROM stat_data WHERE date = ?";
            $r = $DB->rows($sql, $d."-".$i);
            if(isset($r[0]['cnt'])) {
                $res[$n]['cnt'] = $r[0]['cnt'];
                $res[$n]['_day'] = $i;
...
$graphStyle[1]['type'] 	= "frl";
$graphStyle[2]['type'] 	= "emp";
...
if($imgHeight>200) {
    $iFMperPX = (!$bYear)?800:(800*10); //масштаб
    $imgHeight = 0;
    for ($i=1; $i<=2; $i++) {
    	$res = getOP($graphStyle[$i]['type'], $date_from, $date_to, $bYear, $graphStyle[$i]['addit']);
    	$aTemp = $res;
