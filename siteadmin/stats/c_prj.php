...
$sql = "SELECT prj, prj_offers, extract(day from date) as day FROM stat_data_by_spec WHERE date>=? AND date<=? AND spec=?";
$q = $DB->rows($sql, .$syear."-".$smonth."-01", $syear."-".$smonth."-".$days, $sspec);
$is_null = 1;
if($sspec!=0) {
    if($q) {
        foreach($q as $s) {
            if($s['prj']!=0 || $s['prj_offers']!=0) $is_null = 0;
            $data1[$s['day']] = $s['prj'];
            $data2[$s['day']] = $s['prj_offers'];
        }
    }
}
