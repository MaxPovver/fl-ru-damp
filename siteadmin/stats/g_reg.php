...
function getOP($op, $date_from='2006-10-10', $date_to='now()', $bYear=false) {
	...
	if ($bYear) {
		$sql = "SELECT count(*) as cnt, to_char(reg_date,'MM') as _day FROM $op WHERE reg_date >= '".$date_from."' AND reg_date < '".$date_to."'    GROUP BY to_char(reg_date,'MM') ORDER BY to_char(reg_date,'MM')";
	}
	else {
		$sql = "SELECT COUNT(*) as cnt, extract(day from reg_date) as _day FROM $op WHERE reg_date >= '".$date_from."' AND reg_date < '".$date_to."'::date+'1day'::interval  GROUP BY _day ORDER BY  _day";
	}
//	echo $sql.'<br>';
	$res = $DB->rows($sql);
	return $res;
...
$graphStyle[1]['type'] 	= "freelancer";
$graphStyle[2]['type'] 	= "employer";
...
//print_r($graphValues2);
$imgHeight += $graphStyle[0]['max'] + count($graphValues)*$hbl+40; //прибавляем промежутки к максимальной высоте графика
...
$graphStyle[1]['text'] 	= 'Фрилансеры';
$graphStyle[2]['text'] 	= 'Работадатели';
