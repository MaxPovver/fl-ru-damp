...
function getOP($op, $date_from='2006-10-10', $date_to='now()', $bYear=false) {
	...
	if ($bYear) {
        switch($op) {
            case '1':
                $sql = "SELECT count(o.*) as cnt, to_char(o.op_date,'MM') as _day 
                        FROM account_operations as o,
                             account_transaction as t,
                             freelancer as f 
                        WHERE op_date>='$date_from' AND 
                              op_date<'$date_to' AND 
                              op_code=26 AND 
                              o.id=t.opid AND 
                              t.user_id=f.uid
                        GROUP BY to_char(o.op_date,'MM') ORDER BY to_char(o.op_date,'MM')
                        ";
                break;
            case '2':
                $sql = "SELECT count(*) as cnt, to_char(op_date,'MM') as _day  
                        FROM account_operations  
                        WHERE op_date>='$date_from' AND 
                              op_date<'$date_to' AND 
                              op_code=27 
                        GROUP BY to_char(op_date,'MM') ORDER BY to_char(op_date,'MM')
                        ";
                break;
            case '3':
                $sql = "SELECT count(o.*) as cnt, to_char(o.op_date,'MM') as _day 
        ...
        switch($op) {
            case '1':
                $sql = "SELECT count(o.*) as cnt, extract(day from o.op_date) as _day   
                        FROM account_operations as o,
                             account_transaction as t,
                             freelancer as f 
                        WHERE op_date>='$date_from' AND 
                              op_date<'$date_to'::date+'1day'::interval AND 
                              op_code=26 AND 
                              o.id=t.opid AND 
                              t.user_id=f.uid
                        GROUP BY _day ORDER BY _day
                        ";
                break;
            case '2':
                $sql = "SELECT count(*) as cnt, extract(day from op_date) as _day   
                        FROM account_operations  
                        WHERE op_date>='$date_from' AND 
                              op_date<'$date_to'::date+'1day'::interval AND 
                              op_code=27 
...
$graphStyle[0]['color'] = imagecolorallocate($image, 0, 0, 0); //Всего
$graphStyle[1]['color'] = imagecolorallocate($image, 103, 135, 179); //Фрилансер - PRO в подарок
$graphStyle[2]['color'] = imagecolorallocate($image, 111, 177, 92); //Фрилансер - главная в подарок
$graphStyle[3]['color'] = imagecolorallocate($image, 179, 36, 36); //Работадатель - PRO в подарок
$graphStyle[4]['color'] = imagecolorallocate($image, 0, 103, 56); //Работадатель - 85FM в подарок
$graphStyle[0]['text'] 	= 'Всего';
$graphStyle[1]['text'] 	= 'Ф - PRO';
$graphStyle[2]['text'] 	= 'Ф - главная';
$graphStyle[3]['text'] 	= 'Р - PRO';
$graphStyle[4]['text'] 	= 'Р - FM бонус';
