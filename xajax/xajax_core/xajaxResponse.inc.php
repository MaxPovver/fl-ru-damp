...
//					$newQueryPart = rawurlencode($_SERVER['QUERY_STRING']);
        ...
        $_newQueryPart = $_SERVER['QUERY_STRING'];

        if (strstr($_newQueryPart, '&')) {
            $_newQueryPart = explode('&', $_newQueryPart);
            foreach ($_newQueryPart as $pk => $_part) {
                $_newQueryPart[$pk] = htmlspecialchars($_part, ENT_QUOTES);
            }
            $_newQueryPart = implode('&', $_newQueryPart);
        } else {
            $_newQueryPart = htmlspecialchars($_newQueryPart, ENT_QUOTES);
        }

        $newQueryPart = rawurlencode($_newQueryPart);
